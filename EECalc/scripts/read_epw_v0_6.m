function  [ W ] = read_epw_v0_6( file1 )
%read_epw reads an epw format weather file and outputs a structure with
%weather data required for the EECalc software package
%
%W = read_epw (filein) will read in an epw (energy plus weather) format hourly weather
...
for I=1:8
    ...
    theta(:,I)=acos(cos(beta).*cos(gamma(:,I).*pi./180).*sin(sigma)+sin(beta).*cos(sigma));  % ancle of incidence of sun's rays on surface in rad
